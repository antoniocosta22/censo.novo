...
<?php 
    $stmt = $pdo->prepare("SELECT * FROM usuarios WHERE id = $func");
    $stmt->execute();
    if($stmt->rowCount() > 0){
            ...
            <div class="detalhes_frota">
                <div class="recentesobs_frota">
                    <div class="cardheader_frota">
                        <h3>Observações - <?=ucwords(strtolower($dados['nome']))?> - <?=$mesatual?>/<?=$anoatual?></h3>
                    ...
                    <table id="obs">
                        ...
                        <tbody>
                        <?php 
                        $frota = $pdo->prepare(" SELECT * FROM obs WHERE func = $func AND MONTH(data_ocor)=$mesatual AND YEAR(data_ocor)=$anoatual ");
                        $frota->execute();
                        ...
                        ?>
                            <tr style="border-bottom: 1px solid rgb(0, 140, 255);">
                                ...
                                <td>
                                    <?php
                                    if ($user == '87'){
                                        echo "<a class='delete' href='principal.php?p=metas/deletar_obs.php&obss=$carros[id]' target='_blank'><i class='fas fa-trash'></i></a>";
                                    }?>
                                    </td>
                                <style>
                                    .delete {
