...
<thead>
    ...
    if($gvend == 'S' OR $supervend == 'S'){
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n' AND confirmada = 's' AND equip <> 'MG' AND equip <> 'MP' AND cidade = '$cid_sigla' ORDER BY data_cad asc");

    }else {
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n' AND confirmada = 's' AND equip <> 'MG' AND equip <> 'MP' and vend = $user  AND cidade = '$cid_sigla' ORDER BY data_cad asc");
    ...
    <tbody>
        ...
        <!-- seleciona quem confirmou a venda -->

            <?php
                $user_conf = $pdo->prepare("SELECT nome FROM usuarios WHERE id = $vend_paga_n[usr_confir] ");
                $user_conf->execute();
                    while($conf_user = $user_conf->fetch(PDO::FETCH_ASSOC)){
                        $user_c = $conf_user['nome'];
                    }
            ?>
            <td ><span class="status a" title="CONFIRMADA POR <?=$user_c?>" style="cursor: pointer;"><?=$vend_paga_n['id'];?></span></td>
            ...
            <td >
            <?php
                $user_vend = $pdo->prepare("SELECT nome, gvend, supervend  FROM `usuarios` WHERE id = $vend_paga_n[vend] ");
                $user_vend->execute();
                    while($vend_user = $user_vend->fetch(PDO::FETCH_ASSOC)){
                        $vend_usr = $vend_user['nome'];
                        $gvend = $vend_user['gvend'];
                        $supervend = $vend_user['supervend'];
                    }
            ...
            <td style="text-align: center;">
                <div class="dropdown">
                    ...
                    <a href="principal.php?p=vendas/detalhes_venda.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=nao_instaladas.php" target="_blank">Detalhes</a>
                    <!-- <a href="#">Recibo</a> -->
                    <!-- <a href="#">Confirmar</a> -->
                   ...
                   <?php
                    if ($gvend == 'S'){?>
                          <a href="principal.php?p=vendas/editar_<?=$redireciona?>.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=nao_instaladas.php">Editar</a>
