...
<thead>
    ...
    if($gvend == 'S' OR $supervend == 'S'){
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND indicacao = 's' AND cidade = '$cid_sigla' ORDER BY data_cad asc");

    }else {
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND indicacao = 's' and vend = $user  AND cidade = '$cid_sigla' ORDER BY data_cad asc");
    ...
    <tbody>
        <tr style="border-bottom: 1px solid blue; font-weight: bold; text-align: center;">
            <td ><span class="status n"><?=$vend_paga_n['id'];?></span></td>
            ...
            ?>
            <span class="status <?=$status?>" style="font-size: 11px; cursor: pointer;" title="Instalação solicitada dia <?=$dtmov?> completando hoje <?=$prazo?>">	 
                    <?=$dtmov?>
            ...
            <td style="text-align: center;">
                <div class="dropdown">
                    ...
                    <a href="principal.php?p=vendas/detalhes_venda.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=indicadas.php" target="_blank">Detalhes</a>
                    <!-- <a href="#">Recibo</a> -->
                    <!-- instala a venda -->
                    <a href="principal.php?p=vendas/instalar.php&venda=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>">Instalar</a>
                    <!-- sql para buscar as permissões do usuário -->
                    <?php
                    ...
                    $stmt->execute();
                        while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
                            ...
                            // se for gerente de vendas, executa as operações abaixo
                                if ($gvend == 'S'){?>
                            <!-- editar venda -->
                            <a href="principal.php?p=vendas/editar_<?=$redireciona?>.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=indicadas.php">Editar</a>
                            <!-- cancelar venda -->
                            ...
                            <?php }
                            ?>
                            <!-- se a indicação ainda não tiver sido confirmada, mostra o botão -->
                            <?php 
                                if ($vend_paga_n[confirmada] != 's'){?>
                                <a href="" onClick="javascript:window.open ('estrutura/vendas/confirmar.php?id=<?php print $vend_paga_n[id];?>', '', 'toolbar=yes,scrollbars=yes,resizable=yes,top=170,left=250,width=882,height=400')">Confirmar</a>
                            <?php }?>
                    </div>
                </div>
            </td>
