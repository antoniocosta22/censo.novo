...
<thead>
    ...
    if($gvend == 'S' OR $supervend == 'S'){
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n' AND confirmada = 'n' AND equip <> 'MG' AND equip <> 'MP' AND equip <> 'KP' AND cidade = '$cid_sigla' ORDER BY data_cad asc");

    }else {
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n' AND confirmada = 'n' AND equip <> 'MG' AND equip <> 'MP' AND equip <> 'KP' AND vend = $user  AND cidade = '$cid_sigla' ORDER BY data_cad asc");
        ...
        <tr style="border-bottom: 1px solid blue; font-weight: bold; text-align: center;">
            ...
            <td style="text-align: center;">
                <div class="dropdown">
                    ...
                    <a href="principal.php?p=vendas/detalhes_venda.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=nao_confirmadas.php" target="_blank">Detalhes</a>
                    <!-- <a href="#">Recibo</a> -->
                    <!-- confirmar venda -->
                    ...
                    ?>    
                       <!-- se o funcionário for gerente, executa abaixo -->
                    <?php
                    if ($gvend == 'S'){?>
                          <a href="principal.php?p=vendas/editar_<?=$redireciona?>.php&cliente=<?=$vend_paga_n["id"]?>&c=<?=$cid_sigla?>&pg=nao_confirmadas.php">Editar</a>
