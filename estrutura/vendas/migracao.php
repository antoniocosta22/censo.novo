...
<thead>
    ...
    if($gvend == 'S' OR $supervend == 'S'){
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n'  AND equip ='MG' AND cidade = '$cid_sigla' ORDER BY data_cad asc");

    }else {
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n' AND equip ='MG' and vend = $user  AND cidade = '$cid_sigla' ORDER BY data_cad asc");
        ...
        <tr style="border-bottom: 1px solid blue; font-weight: bold; text-align: center;">
            ...
            <td style="text-align: center;">
                <div class="dropdown">
                    ...
                    <a href="principal.php?p=vendas/detalhes_venda.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=migracao.php" target="_blank">Detalhes</a>
                    <!-- <a href="#">Recibo</a> -->
                    <!-- instala a venda -->
                    ...
                    if ($gvend == 'S'){?>
                          ...
                          <a href="principal.php?p=vendas/editar_<?=$redireciona?>.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=migracao.php">Editar</a>
    ...
    <?php } } else{
        ...
        Nenhuma migração de tecnologia!
        </td>
        </tr>
        </tbody>";
