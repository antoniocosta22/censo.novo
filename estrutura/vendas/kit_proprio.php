...
<thead>
    ...
    if($gvend == 'S' OR $supervend == 'S'){
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n' AND equip ='KP' AND cidade = '$cid_sigla' ORDER BY data_cad asc");

    }else {
        $vendas_nins = $pdo->prepare("SELECT * FROM `vendas` WHERE consolidada = 'n' AND instalada = 'n' AND equip ='KP' and vend = $user  AND cidade = '$cid_sigla' ORDER BY data_cad asc");
    ...
    <tbody>
        <tr style="border-bottom: 1px solid blue; font-weight: bold; text-align: center;">
            <td ><span class="status mig_t"><?=$vend_paga_n['id'];?></span></td>
            ...
            <td style="text-align: center;">
                <div class="dropdown">
                    ...
                    <a href="principal.php?p=vendas/detalhes_venda.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=kit_proprio.php" target="_blank">Detalhes</a>
                    <!-- <a href="#">Recibo</a> -->
                    <!-- <a href="#">Confirmar</a> -->
                    <!-- instala a venda -->
                    ...
                    $stmt->execute();
                        while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
                            ...
                            if ($gvend == 'S'){?>
                                ...
                                <a href="principal.php?p=vendas/editar_<?=$redireciona?>.php&cliente=<?=$vend_paga_n['id']?>&c=<?=$cid_sigla?>&pg=kit_proprio.php">Editar</a>
