...
.dropdown:hover .listar_acoes {
  ...
  right: 0;
    ...
    <div class="cardbox">
<?php 
    $usuario_cx = $pdo->prepare("SELECT nome FROM usuarios WHERE id = $id_cx ");
    $usuario_cx->execute();
        while($user_cx = $usuario_cx->fetch(PDO::FETCH_ASSOC)){
                $user_caixa = $user_cx['nome'];
        }
            ...
            <div class="detalhes_frota">
                <div class="recentesobs_frota">
                    ...
                    <table id="ferramentas" >
                        ...
                        <tbody>
                        <?php 
                            $stmt = $pdo->prepare("SELECT id_ferramenta, tombo, status, ferramenta, responsavel, cidade, data_cad FROM ferramenta_cadastro WHERE responsavel = $id_cx ORDER BY id_ferramenta");
                            $stmt->execute();
                            if($stmt->rowCount() > 0){
                        ...
                        ?>
                            <tr>
                                ...
                                <div class="dropdown">
                                    ...
                                    <a href="principal.php?p=tools/tools_ocorrencia.php&tool=<?=$id_ferra?>&pg=patrimonio/caixa_usuarios&cx_us=<?=$id_cx?>">Ocorrências</a>
                                    ...
                                    ?>
                                    <a href="principal.php?p=tools/<?=$pagina?>&t=<?=$dados['id_ferramenta'];?>"><?=$nome_func?></a>
                                    </div>
                                    </div>
                                </td>
                                </td>
